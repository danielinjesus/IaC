  ...
  resource "azuread_user" "rag_users" {
    ...
    user_principal_name = format("kt%02d%s", count.index, var.tenant_email)
    display_name        = format("kt%02d", count.index)
  ...
  resource "azurerm_resource_group" "rg" { # 01_RG부터 user_count_RG까지 생성
    count    = var.user_count
    name     = format("kt%02d_RG", count.index)
